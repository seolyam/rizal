  ...
  return (
    <div className="grid gap-10 md:grid-cols-[220px_1fr]">
      <aside className="md:sticky md:top-24">
        <ScrollArea className="max-h-[70vh] rounded-3xl border border-border/60 bg-card/70 p-4">
      ...
      <div className="relative space-y-12 border-l border-border/40 pl-10">
        <span className="absolute -left-[9px] top-0 h-full w-[3px] bg-gradient-to-b from-secondary via-border/70 to-transparent" />
        {grouped.map(([year, items]) => (
          <motion.section
            ...
            viewport={prefersReducedMotion ? undefined : { once: true, margin: "-20% 0px -20% 0px" }}
            className="scroll-mt-24 space-y-6"
          >
            <motion.div
              variants={prefersReducedMotion ? undefined : fadeUp}
              className="inline-flex items-baseline gap-3 rounded-full border border-border/60 bg-background px-4 py-2 font-serif text-2xl font-semibold"
            >
            ...
            <div className="grid gap-6 md:grid-cols-2">
              {items.map((event) => (
                <motion.article
                  ...
                  variants={prefersReducedMotion ? undefined : fadeUp}
                  className={cn(
                    "rounded-3xl border border-border/60 bg-card p-6 shadow-sm transition",
